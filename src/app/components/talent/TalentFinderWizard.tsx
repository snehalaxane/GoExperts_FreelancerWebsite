...
import { 
  ...
  UserPlus, Tag, Search, CheckCircle, Info, Building, Camera, User
} from 'lucide-react';
...
export default function TalentFinderWizard({ onClose, onComplete }: TalentFinderWizardProps) {
    ...
    const fetchData = async () => {
      try {
        const [stepsRes, categoriesRes, skillsRes] = await Promise.all([
          api.get('/cms/registration-steps?module=talent_finder'),
  ...
  const canProceed = () => {
    ...
    if (currentStepData.type === 'multi-selection') {
        // For talent finder, preferences might be optional but skills usually aren't
        if (currentStepData.field === 'preferences') return true;
  ...
  const getIcon = (iconName: string) => {
    const icons: any = {
      ...
      ListChecks, Type, CircleDashed, UserPlus, Tag, Building, Camera, User
    };
    return icons[iconName] || User;
  };

  const selectedCategoryId = useMemo(() => {
    const type = answers['role'];
    if (!type) return null;
    const matched = categories.find(c => c.name.toLowerCase() === type.toLowerCase());
    return matched?._id || null;
  }, [answers.role, categories]);
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 z-[45] flex items-center justify-center bg-black/95 backdrop-blur-xl">
        <div className="text-white text-xl font-bold flex flex-col items-center gap-4">
          ...
          <p className="animate-pulse">Preparing Talent Network...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="w-80 flex-shrink-0 py-12 flex flex-col h-full border-r border-neutral-800/50 pr-8">
          <div className="mb-12">
            <h2 className="text-3xl font-black text-white mb-2 flex items-center gap-3">
              <UserPlus className="w-8 h-8 text-[#F24C20]" />
              Find Talent
            </h2>
            <p className="text-neutral-400 text-lg">Hire your perfect expert</p>
          </div>
          ...
          <div className="mt-8 pt-8 border-t border-neutral-800">
             <div className="flex justify-between items-end mb-2">
                <span className="text-neutral-500 text-xs font-bold uppercase tracking-wider">Progress</span>
                <span className="text-white text-lg font-black">{totalSteps > 0 ? Math.round((currentStep/totalSteps)*100) : 0}%</span>
             </div>
        ...
        <div className="flex-1 flex flex-col py-12 h-full">
           ...
           <div className="flex-1 relative overflow-y-auto pr-6 custom-scrollbar pb-32">
              <AnimatePresence mode="wait">
                 {currentStepData && (
                   <motion.div
                     key={currentStepData._id || currentStep}
                   ...
                   >
                     ...
                     {/* Search Bar for Roles & Skills */}
                     {(currentStepData.field === 'role' || currentStepData.field === 'skills') && (
                       <div className="relative max-w-xl group">
                          ...
                          <input 
                            type="text"
                            placeholder={`Search ${currentStepData.field}...`}
                            value={searchQuery}
                     ...
                     {currentStepData.type === 'single-selection' && (
                       <div className={`grid ${currentStepData.field === 'role' || filteredOptions.length > 4 ? 'grid-cols-2 lg:grid-cols-3' : 'grid-cols-1 md:grid-cols-2'} gap-4`}>
                       ...
                       <div className="space-y-6">
                           ...
                           {currentStepData.field === 'skills' && (
                               <div className="flex items-center gap-2 text-neutral-500 bg-neutral-900/50 p-4 rounded-xl max-w-fit">
                                  ...
                                  <span className="text-sm font-medium">Use the search bar above to quickly find specific technologies.</span>
                               </div>
                           )}
                       </div>
                     )}
