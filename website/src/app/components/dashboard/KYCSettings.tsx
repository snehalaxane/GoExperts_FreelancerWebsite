...
export default function KYCSettings({ userRole }: KYCSettingsProps) {
    ...
    const handleNextStep1 = () => {
        let newErrors: any = {};
        if (!formData.identity.email.includes('@')) {
            newErrors.email = "Please enter a valid email address.";
        }
        const mobileRegex = /^\d{10}$/;
        if (!mobileRegex.test(formData.identity.mobile)) {
    ...
    const StatusCard = () => (
        <div className={`p-2 rounded-[2.5rem] border mb-8 flex items-center justify-between ${
            ...
            <div className="flex items-center gap-6">
                ...
                <div>
                    <h3 className={`text-sm font-black tracking-tighter uppercase ${isDarkMode ? 'text-red-500' : 'text-neutral-900'}`}>
    ...
    return (
        <div className="space-y-6">
            ...
            {(kycData?.status === 'unverified' || kycData?.status === 'rejected') && (
                <div className={`p-5 rounded-[3rem] border ${isDarkMode ? 'bg-neutral-900/50 border-neutral-800' : 'bg-white border-neutral-200 shadow-2xl'}`}>
                    <ProgressHeader />
                    ...
                    <div className="min-h-[400px]">
                        <AnimatePresence mode="wait">
                            ...
                            {step === 1 && (
                                ...
                                >
                                    <div>
                                        <h4 className={`text-xl mb-2 font-black ${isDarkMode ? 'text-gray-900' : 'text-gray-900'}`}>1. Basic Identity</h4>
                                        <p className="text-xs text-neutral-500 font-bold uppercase tracking-widest">Foundational verification for regulatory compliance.</p>
                                    </div>
                                    <div className="grid md:grid-cols-2 gap-6">
                                        ...
                                        <InputField label="Date of Birth" type="date" placeholder="YYYY-MM-DD" value={formData.identity.dob} onChange={(v: string) => setFormData({...formData, identity: {...formData.identity, dob: v}})} icon={Calendar} />
                                        <InputField label="Mobile Number" value={formData.identity.mobile} onChange={(v: string) => { setFormData({...formData, identity: {...formData.identity, mobile: v}}); setErrors((prev: any) => ({...prev, mobile: undefined})); }} icon={ShieldCheck} error={errors.mobile} />
                                        <InputField label="Email Address" value={formData.identity.email} onChange={(v: string) => { setFormData({...formData, identity: {...formData.identity, email: v}}); setErrors((prev: any) => ({...prev, email: undefined})); }} icon={AlertCircle} error={errors.email} />
                                    </div>
                                    <div className="flex justify-end pt-6">
                                        <NavButton label="Next Phase" onClick={handleNextStep1} icon={ArrowRight} />
                                    </div>
                                </motion.div>
                            )}
                            ...
                            {step === 2 && (
                                ...
                                >
                                    <div>
                                        <h4 className={`text-xl mb-2 font-black ${isDarkMode ? 'text-gray-900' : 'text-gray-900'}`}>2. Government Credentials</h4>
                                        <p className="text-xs text-neutral-500 font-bold uppercase tracking-widest">Official state-issued identification documents.</p>
                                    </div>
                             ...
                             {step === 3 && (
                                ...
                                >
                                    {userRole === 'investor' ? (
                                        <>
                                            <div>
                                                <h4 className={`text-xl mb-2 font-black ${isDarkMode ? 'text-gray-900' : 'text-gray-900'}`}>3. Investment Thesis</h4>
                                        ...
                                        <>
                                            <div>
                                                <h4 className={`text-xl mb-2 font-black ${isDarkMode ? 'text-gray-900' : 'text-gray-900'}`}>3. Startup Infrastructure</h4>
                                                <p className="text-sm text-neutral-500 font-bold uppercase tracking-widest">Details about your vision and registered entity.</p>
                                            </div>
                                            <div className="grid md:grid-cols-2 gap-6">
                             ...
                             {step === 4 && (
                                ...
                                >
                                    <div>
                                        <h4 className={`text-xl font-black ${isDarkMode ? 'text-gray-900' : 'text-gray-900'}`}>4. Trust & Safety Layer</h4>
...
function InputField({ label, value, onChange, icon: Icon, placeholder = "", type = "text", error }: any) {
    return (
        <div className="space-y-2">
            <label className="text-[13px] text-neutral-500 ml-1">{label}</label>
            <div className="relative group">
                <div className="absolute inset-y-0 left-4 flex items-center text-neutral-500 group-focus-within:text-[#F24C20] transition-colors pointer-events-none">
            ...
            {error && <p className="text-xs text-red-500 ml-1 mt-1 font-medium">{error}</p>}
        </div>
    );
}
