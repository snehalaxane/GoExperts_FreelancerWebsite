      ...
      if (res.data.success) {
        ...
        const processed = filtered.map((t: any) => {
          ...
          return {
            ...t,
            id: t._id || t.id,
            role: t.profile?.role_title || t.role || t.title || 'Expert Professional',
...
className="relative p-6 rounded-2xl bg-gradient-to-br from-[#FFEAD4]/60 to-[#044071]/20 border border-[#FFE0C2] hover:border-[#F24C20]/60 transition-all group flex flex-col h-full hover:shadow-lg hover:shadow-orange-500/10"    >
      <div className="flex-1">
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {talent.role && <p className="text-sm text-gray-700 capitalize truncate">{talent.role}</p>}
            {talent.skills && talent.skills.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2">
                {talent.skills.slice(0, 3).map((skill: any, idx: number) => {
                  ...
                  return (
                    <span key={idx} className="px-2 py-0.5 rounded text-[12px] font-medium bg-orange-50 text-orange-600 border border-orange-100">
                      {skillName}
                    </span>
                  );
                })}
              ...
              <div className="flex-1 space-y-8">
                {featuredTalent && (
                  <motion.div
                    ...
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="relative p-6 lg:p-8 rounded-3xl bg-gradient-to-br from-[#FFEAD4]/60 to-[#044071]/20 border border-border overflow-hidden shadow-sm group"
                  >
                    ...
                    <div className="relative flex gap-4 lg:gap-8">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="mb-2 lg:mb-3">
                          ...
                          {featuredTalent.skills && featuredTalent.skills.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-3">
                              {featuredTalent.skills.slice(0, 5).map((skill: any, idx: number) => {
                                const skillName = typeof skill === 'object' ? skill.name : skill;
                                if (!skillName) return null;
                                return (
                                  <span key={idx} className="px-2.5 py-1 rounded-md bg-orange-50 text-orange-600 text-xs font-semibold border border-orange-100">
                                    {skillName}
                                  </span>
                                );
                              })}
